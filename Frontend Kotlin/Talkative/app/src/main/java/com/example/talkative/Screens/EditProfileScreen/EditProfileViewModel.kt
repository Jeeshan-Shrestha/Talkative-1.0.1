...
import com.example.talkative.repository.EditProfileRepository
...
class EditProfileViewModel  @Inject constructor(
    ...
    private  val editProfileRepository: EditProfileRepository): AndroidViewModel(application){

    var item: EditProfileResponse by mutableStateOf(
        EditProfileResponse(message = "", success = false)
    )
    private val _state= MutableStateFlow(LoadingState.IDLE)
    val state = _state.asStateFlow()

    fun PostImage(displayName: String,
                  bio:String,
                  avatar:Uri?,
                  coverPhoto: Uri?,
                  onSuccess:()-> Unit){
        ...
        viewModelScope.launch {
            ...
            try {
                val response = editProfileRepository.CreatePost(
